...
export const useHostMenuStore = defineStore("use-host-menu-store", () => {
  ...
  const getAllConfig = computedAsync<MonoConfig[]>(async () => {
    ...
    function isFederationConfig(v: any): v is MonoConfig {
    ...
    const loaders = Object.entries(allModules).map(async ([file, load]) => {
      try {
        ...
        return isFederationConfig(cfg) ? (cfg as MonoConfig) : null
      } catch (e) {
        return null
      }
    })
  ...
  const menuFederation = computedAsync<SidebarMenu[]>(async () => {
  ...
  const menuList = computed(() => {
    ...
    return [staticDataSidebar, additionalMenu.value, menuFederation.value].reduce((acc, next) => mergeSiblings(acc, next), [])
  }
  );
